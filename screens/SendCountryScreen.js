...
export default function SendCountryScreen({ navigation, route }) {
  const { email, nywila, nchiYake } = route.params || {};
  ...
  function endelea() {
    if (!chaguliwa) return;
    navigation.navigate('PhoneNumber', { email, nywila, nchiYake, nchiTuma: chaguliwa });
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.kichwa}>Which country do you</Text>
      <Text style={styles.kichwa}>send the most money to?</Text>
      <Text style={styles.subKichwa}>You can send to any available country today</Text>
...
const styles = StyleSheet.create({
  ...
  kichwa: { fontSize: 24, fontWeight: 'bold', color: 'white', paddingHorizontal: 20, lineHeight: 30 },
  subKichwa: { fontSize: 13, color: 'rgba(255,255,255,0.6)', paddingHorizontal: 20, marginTop: 8, marginBottom: 20 },
